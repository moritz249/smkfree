...
async function downloadReceiptImage() {
  ...
  const rows = [
    ["Smoke free days", numberFormatter.format(progress.wholeDays)],
    ["Weeks", numberFormatter.format(progress.weeks)],
    ["Months", numberFormatter.format(progress.months)],
    ["Years", numberFormatter.format(progress.years)],
    ["Money not burned", formatCurrency(progress.moneySaved, state.currencyCode)],
    ...
    ["Pack price", formatCurrency(state.pricePerPack, state.currencyCode)],
    ["Cost avoided", formatCurrency(progress.moneySaved, state.currencyCode)],
    ["Estimated life back", formatLifeSaved(progress.estimatedLifeMinutes)],
  ];
  const benefits = milestones.map((milestone) => ({
    done: progress.elapsedHours >= milestone.hours,
    time: milestone.hours < 48 ? `${milestone.hours}h` : `${Math.round(milestone.hours / 24)}d`,
  ...
  function setFont(size, weight = 400) {
    context.font = `${weight} ${size}px "Lucida Console", "Courier New", monospace`;
  }

  function drawRow(label, value, y, size = 25) {
    setFont(size);
  ...
  context.lineWidth = 2;
  context.strokeRect(12, 12, width - 24, estimatedHeight - 24);

  context.textAlign = "center";
  context.font = "italic 700 88px Arial, Helvetica, sans-serif";
  context.fillText("SMKFREE", width / 2, y + 82);
  context.fillRect(padding, y + 100, width - padding * 2, 12);
  y += 160;
  setFont(28);
  context.fillText("QUIT RECEIPT", width / 2, y);
  y += 42;
  setFont(22);
  ...
  benefits.forEach((benefit) => {
    ...
    context.textAlign = "right";
    context.fillText(`${benefit.time}${benefit.done ? "+" : "-"}`, width - padding, y);
    context.textAlign = "left";
    y += 24;
    y = wrapText(benefit.detail, padding, y, width - padding * 2, 17);
    y += 6;
  });
  ...
  y += 22;
  drawBarcodeOnCanvas(context, code, padding, y, width - padding * 2, 66);
  y += 92;
  setFont(18);
  context.fillText(code, padding, y);
  y += 52;
  context.textAlign = "center";
  setFont(22);
  context.fillText("Thank you for choosing fresh air.", width / 2, y);
  y += 42;
  context.fillText("************************", width / 2, y);

  const finalHeight = Math.min(y + padding, estimatedHeight);
  const output = document.createElement("canvas");
  output.width = width * scale;
  output.height = finalHeight * scale;
  output.getContext("2d").drawImage(canvas, 0, 0);

  downloadUrl(output.toDataURL("image/png"), "smoke-free-receipt.png");
}
